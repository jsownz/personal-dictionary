...
def test_letter_swap():
    ...
    assert(letter_swap('hijklmnop') == ['#'], ['!'], [], [], ['1'], [], [],
            ['0'], [])
...
def test_permute_phone():
    assert(permute_phone('0123456789') == ['0123456789', '3456789', '012',
            '6789', '9876543210', '210', 'O123456789', '0l23456789',
            '01Z3456789', '012E456789', '0123A56789', '01234S6789',
            '012345b789', '012345G789', '0123456T89', '0123456L89',
...
def test_permute_music():
    assert(permute_music('Cure') == ['cure', 'Cure', 'eruC', 'CuRe', 'cUrE',
            '(ure', 'cur3'])
    assert(permute_music('Foo Fighters') == ['foo fighters', 'Foo fighters',
            'srethgiF ooF', 'FoO fIgHtErS', 'fOo FiGhTeRs', 'f0o fighters',
            'fo0 fighters', 'foo f!ghters', 'foo fi6hters', 'foo fi9hters',
            'foo fig#ters', 'foo figh+ers', 'foo figh7ers', 'foo fight3rs',
            'foo fighter5', 'foo fighter$'])
...
def test_perm_st_num():
    ...
    assert(perm_st_num('1234') == ['1234', '4321', 'l234', '1Z34', '12E4',
            '123A'])
    assert(perm_st_num('55667') == ['55667', '76655', 'S5667', '5S667',
            '55b67', '55G67', '556b7', '556G7', '5566T', '5566L'])
    assert(perm_st_num('10A') == ['10A', 'A01', 'l0A', '1OA'])

def test_mangle():
    assert(mangle(["spike", "tiger"]) == ['5pike', '$pike', 'sp!ke', 'spik3',
            'spike', 'Spike', 'SpIkE', 'sPiKe', '+iger', '7iger', 't!ger',
            'ti6er', 'ti9er', 'tig3r', 'tiger', 'Tiger', 'TiGeR', 'tIgEr'])
    assert(mangle(["poker", "lakers"]) == ['p0ker', 'pok3r', 'poker', 'Poker',
            'PoKeR', 'pOkEr', '1akers', 'l@kers', 'l4kers', 'lak3rs', 'laker5',
            'laker$', 'lakers', 'Lakers', 'LaKeRs', 'lAkErS'])
